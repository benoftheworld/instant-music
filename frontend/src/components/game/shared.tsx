...
/* ───────────────────── Audio hook for Reversed (inverse) mode ───────────────────── */
export function useReverseAudioPlayer(
  round: GameRound,
  showResults: boolean,
) {
  ...
  useEffect(() => {
    ...
    const playReversed = async () => {
      try {
        ...
        if (cancelled || !mountedRef.current) { context.close(); return; }

        // Reverse PCM samples in each channel in-place
        ...
        gain.gain.value = getEffectiveMusicVolume();
        source.connect(gain);
        ...
        if (cancelled || !mountedRef.current) return;
        setPlayerError("Impossible de charger l'audio en mode inverse (CORS non supporté par la source)");
        setNeedsPlay(false);
      }
    };
  ...
  }, [round.track_id, round.id, showResults, round.preview_url, retry, stopAndCleanup]);

  // Fallback: reversed audio takes longer to load (fetch + decode), allow 6 s
  useEffect(() => {
    if (showResults) return;
    const fallback = setTimeout(() => {
    ...
    return () => clearTimeout(fallback);
  }, [isPlaying, playerError, showResults, round.track_id, round.id, round.preview_url, retry]);

  // Live volume sync via GainNode
  useEffect(() => {
    const onVolumeChange = () => {
      if (gainRef.current) gainRef.current.gain.value = getEffectiveMusicVolume();
    };
    window.addEventListener('music-volume-change', onVolumeChange);
    return () => window.removeEventListener('music-volume-change', onVolumeChange);
  }, []);

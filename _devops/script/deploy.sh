...
# ── Génération du fichier AlertManager (production) ──────────────────────
if [[ "$ENV" == "production" ]]; then
    ALERTMANAGER_TEMPLATE="$DEVOPS_DIR/monitoring/alertmanager/alertmanager-prod.yml.template"
    ALERTMANAGER_OUTPUT="$DEVOPS_DIR/monitoring/alertmanager/alertmanager-prod.yml"
    if [[ -f "$ALERTMANAGER_TEMPLATE" ]]; then
        log_info "Generation de la config AlertManager depuis le template..."
        # Substitution sécurisée via Python : ne jamais sourcer .env.prod
        # (évite l'exécution de valeurs non quotées comme des commandes shell)
        python3 - "$ENV_FILE_DOCKER" "$ALERTMANAGER_TEMPLATE" "$ALERTMANAGER_OUTPUT" <<'PYEOF'
import sys, re

def load_alertmanager_vars(env_path):
    """Lit uniquement les lignes ALERTMANAGER_* du fichier .env."""
    env = {}
    with open(env_path, encoding='utf-8') as f:
        for line in f:
            line = line.rstrip('\r\n').strip()
            if line.startswith('ALERTMANAGER_') and '=' in line:
                key, _, val = line.partition('=')
                env[key.strip()] = val.strip().strip('"').strip("'")
    return env

env_path, tpl_path, out_path = sys.argv[1], sys.argv[2], sys.argv[3]
env = load_alertmanager_vars(env_path)
...
with open(out_path, 'w', encoding='utf-8') as f:
    f.write(content)
print(f'alertmanager-prod.yml generated ({len(env)} variables substituted)')
PYEOF
        log_success "alertmanager-prod.yml genere."
    else
        log_warn "Template AlertManager introuvable ($ALERTMANAGER_TEMPLATE)."
    fi
fi

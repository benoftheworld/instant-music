...
class TestYouTubeServiceSearchPlaylists(BaseServiceUnitTest):
    ...
    def test_api_error_returns_empty(self, mock_cache):
        mock_cache.get.return_value = None
        from apps.playlists.youtube_service import YouTubeService, YouTubeAPIError
        svc = YouTubeService()
        svc.api_key = "key"
...
class TestYouTubeServiceSearchMusicVideos(BaseServiceUnitTest):
    ...
    def test_success(self, mock_cache):
        ...
        svc.api_key = "key"
        with patch.object(svc, "_make_request") as mock_req, \
             patch.object(svc, "_get_video_details", return_value={"v1": {"duration_ms": 180000}}):
            mock_req.return_value = {
                "items": [{
                    "id": {"videoId": "v1"},
                    "snippet": {
...
class TestYouTubeServiceGetPlaylistTracks(BaseServiceUnitTest):
    ...
    def test_success(self, mock_cache):
        ...
        svc.api_key = "key"
        with patch.object(svc, "_make_request") as mock_req, \
             patch.object(svc, "_get_video_details", return_value={
                 "v1": {"duration_ms": 180000, "embeddable": True}
             }):
            mock_req.return_value = {
                "items": [{
                    "snippet": {
    ...
    def test_skips_deleted_videos(self, mock_cache):
        ...
        with patch.object(svc, "_make_request") as mock_req, \
             patch.object(svc, "_get_video_details", return_value={
                 "v1": {"duration_ms": 0, "embeddable": True}
             }):
            mock_req.return_value = {
                "items": [{
    ...
    def test_skips_non_embeddable(self, mock_cache):
        ...
        with patch.object(svc, "_make_request") as mock_req, \
             patch.object(svc, "_get_video_details", return_value={
                 "v1": {"duration_ms": 180000, "embeddable": False}
             }):
            mock_req.return_value = {
                "items": [{
                    "snippet": {

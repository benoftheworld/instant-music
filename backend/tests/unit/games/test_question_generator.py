...
class TestQuestionGeneratorFetchTracks(BaseServiceUnitTest):
    ...
    def test_fetch_tracks_success(self, mock_deezer):
        mock_deezer.get_playlist_tracks.return_value = [
            {"track_id": i, "name": f"T{i}", "artists": [f"A{i}"]}
            for i in range(10)
        ]
        svc = self._make_svc()
...
class TestQuestionGeneratorGenerateForMode(BaseServiceUnitTest):
    ...
    @patch.object(
        __import__("apps.games.services.question_generator", fromlist=["QuestionGeneratorService"]).QuestionGeneratorService,
    ...
    def test_classique_title(self, mock_gen):
        ...
        result = svc._generate_for_mode("classique", track, [track], guess_target="title")
    ...
    def test_classique_artist(self, mock_gen):
        svc = self._make_svc()
        track = self._make_track()
        result = svc._generate_for_mode("classique", track, [track], guess_target="artist")
        assert result is not None

    def test_generation_mode(self):
        svc = self._make_svc()
        track = self._make_track()
        with patch.object(svc, "_generate_year_question", return_value={"q": "year"}) as m:
    ...
    def test_paroles_mode(self):
        svc = self._make_svc()
        track = self._make_track()
        with patch.object(svc, "_generate_lyrics_question", return_value={"q": "lyrics"}) as m:
            result = svc._generate_for_mode("paroles", track, [track])
            m.assert_called_once()

    def test_karaoke_mode(self):
        svc = self._make_svc()
        track = self._make_track()
        with patch.object(svc, "_generate_karaoke_question", return_value={"q": "k"}) as m:
            result = svc._generate_for_mode("karaoke", track, [track])
            m.assert_called_once()

    def test_rapide_adds_audio_duration(self):
        svc = self._make_svc()
        track = self._make_track()
        with patch.object(svc, "_generate_guess_title_question", return_value={"extra_data": {}}):
            result = svc._generate_for_mode("rapide", track, [track], guess_target="title")
    ...
    def test_lent_adds_slow_effect(self):
        ...
        track = self._make_track()
        with patch.object(svc, "_generate_guess_title_question", return_value={"extra_data": {}}):
            result = svc._generate_for_mode("mollo", track, [track], guess_target="title")
            assert result["extra_data"]["audio_effect"] == "slow"

    def test_unknown_mode_defaults_title(self):
        svc = self._make_svc()
        track = self._make_track()
        with patch.object(svc, "_generate_guess_title_question", return_value={"q": "t"}) as m:
...
class TestGenerateYearQuestion(BaseServiceUnitTest):
    ...
    def test_year_question_success(self, mock_deezer):
        ...
        track = {"track_id": "1", "name": "Song", "artists": ["Artist"],
                 "preview_url": "url", "album_image": "img"}
...
class TestGenerateLyricsQuestion(BaseServiceUnitTest):
    ...
    def test_success(self, mock_gl, mock_clq):
        ...
        mock_clq.return_value = ("snippet ___", "correct", ["correct", "w1", "w2", "w3"])
        from apps.games.services.question_generator import QuestionGeneratorService
...
class TestGenerateQuestions(BaseServiceUnitTest):
    ...
    def test_generate_questions(self):
        ...
        svc = QuestionGeneratorService()
        tracks = [
            {"track_id": str(i), "name": f"Song{i}", "artists": [f"A{i}"],
             "preview_url": f"u{i}", "album_image": f"img{i}"}
            for i in range(10)
        ]
        with patch.object(svc, "_fetch_tracks", return_value=tracks):
            with patch.object(svc, "_generate_for_mode", side_effect=lambda *a, **kw: {"q": "ok"}):

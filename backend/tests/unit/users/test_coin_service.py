...
class TestCoinService(BaseServiceUnitTest):
    ...
    @patch("apps.users.coin_service.User")
    @patch("apps.users.coin_service.transaction")
    def test_add_coins_negative_returns_balance(self, mock_tx, mock_user_cls, mock_balance):
    ...
    @patch("apps.users.coin_service.User")
    @patch("apps.users.coin_service.transaction")
    def test_deduct_coins_zero_returns_balance(self, mock_tx, mock_user_cls, mock_balance):
        from apps.users.coin_service import deduct_coins
    ...
    @patch("apps.users.coin_service.User")
    @patch("apps.users.coin_service.transaction")
    def test_deduct_coins_negative_returns_balance(self, mock_tx, mock_user_cls, mock_balance):
        from apps.users.coin_service import deduct_coins
    ...
    def test_get_balance_returns_value(self, mock_user_cls):
        mock_user_cls.objects.filter.return_value.values_list.return_value.first.return_value = 200
    ...
    def test_get_balance_none_returns_zero(self, mock_user_cls):
        ...
        mock_user_cls.objects.filter.return_value.values_list.return_value.first.return_value = None

...
class GameService:
    ...
    def submit_answer(
        ...
        GamePlayer.objects.filter(pk=player.pk).update(
            score=F("score") + points,
            consecutive_correct=player.consecutive_correct,
        )
    ...
    def finish_game(self, game: Game) -> Game:
        ...
        # Verrou pessimiste pour éviter la double terminaison (race condition)

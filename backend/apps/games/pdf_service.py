...
def _draw_page(canvas, doc, room_code: str, game_name: str) -> None:
    ...
    canvas.setFillColor(C_WHITE)
    canvas.drawRightString(PAGE_W - MARGIN, PAGE_H - HEADER_H + 5 * mm, "Résultats de partie")
...
def generate_results_pdf(
    ...
    S = {
        ...
        "title":    _s("title",  fontSize=20, textColor=C_DARK,      fontName="Helvetica-Bold", spaceAfter=2),
        ...
        "bold":     _s("bold",   fontSize=9,  textColor=C_DARK_TEXT,  fontName="Helvetica-Bold"),
        "bold_sm":  _s("boldsm", fontSize=8,  textColor=C_DARK_TEXT,  fontName="Helvetica-Bold"),
        ...
        "center_w": _s("ctrw",   fontSize=9,  textColor=C_WHITE,      alignment=1, fontName="Helvetica-Bold"),
    ...
    # Info grid (2 columns)
    meta_pairs: list[tuple[str, str]] = [("Mode", mode_display), ("Rounds", str(num_rounds))]
    if answer_mode:
    ...
    if meta_rows:
        mt = Table(meta_rows, colWidths=[COL_W * 0.16, COL_W * 0.34, COL_W * 0.16, COL_W * 0.34])
    ...
    if top3_by_rank:
        ...
        for col_idx, rank in enumerate(podium_order):
            ...
            slot = Table(
                [
                    [Paragraph(f"<b>{_medal(rank)}</b>", _s(f"pm_r{rank}", fontSize=11, fontName="Helvetica-Bold", textColor=col_color, alignment=1))],
                    [Paragraph(f"<b>{uname}</b>",         _s(f"pm_u{rank}", fontSize=9,  fontName="Helvetica-Bold", textColor=C_DARK_TEXT, alignment=1))],
                    [Paragraph(f"{score} pts",             _s(f"pm_s{rank}", fontSize=10, fontName="Helvetica-Bold", textColor=C_WHITE,     alignment=1))],
                ],
                colWidths=[COL_W / 3 - 4],
                rowHeights=[14, 13, bar_heights[col_idx] * mm / 25.4 * 72],
            )
            slot.setStyle(TableStyle([
                ("BACKGROUND",    (0, 0), (0, 1), colors.HexColor("#F9F8FF")),
    ...
    if rounds:
        ...
        for rd in rounds:
            ...
            sorted_ans = sorted(
                answers,
                key=lambda a: (-a.get("points_earned", 0), a.get("response_time", 9999)),
            )
            try:
                min_time = min(float(a.get("response_time", 9999)) for a in sorted_ans) if sorted_ans else None
            except Exception:
                min_time = None
            ...
            hdr = Table(
                [[
                    Paragraph(f"<b>Round {rnum}</b>  ·  {track}",
                              _s(f"rh{rnum}a", fontSize=9, fontName="Helvetica-Bold", textColor=C_WHITE)),
                    Paragraph(artist,
                              _s(f"rh{rnum}b", fontSize=8, textColor=colors.HexColor("#C4B5FD"))),
                    Paragraph(f"Bonne réponse : <b>{correct}</b>",
                              _s(f"rh{rnum}c", fontSize=8, textColor=colors.HexColor("#86EFAC"))),
                ]],
            ...
            if sorted_ans:
                col_hdr_sty = _s("ahdr", fontSize=8, fontName="Helvetica-Bold", textColor=C_DARK_TEXT)
                ans_rows_data = [[
                ...
                for row_i, a in enumerate(sorted_ans, start=1):
                    ...
                    ans_rows_data.append([
                        ...
                        Paragraph(
                            f"<b>{a.get('username','?')}</b>" if is_fastest else a.get("username", "?"),
                            S["bold_sm"] if is_fastest else S["body_sm"],
                        ),
                        Paragraph(ans_text, txt_sty),
                        Paragraph(f"+{a.get('points_earned',0)}", S["bold_sm"] if is_ok else S["grey"]),
                        Paragraph(time_str, S["grey"]),
                        Paragraph(streak_s, S["grey"]),
                    ])

                    if is_fastest:
                        row_bg_cmds.append(("BACKGROUND", (0, row_i), (-1, row_i), C_FASTEST))
                    elif is_ok:
                        row_bg_cmds.append(("BACKGROUND", (0, row_i), (-1, row_i), C_CORRECT))
                    else:
                        row_bg_cmds.append(("BACKGROUND", (0, row_i), (-1, row_i), C_WRONG))
    ...
    score_lines = [
        [Paragraph("<b>Base</b>",         S["bold_sm"]),
         Paragraph(f"max({SCORE_MIN_CORRECT}, {SCORE_BASE_POINTS} − temps × {SCORE_TIME_PENALTY_PER_SEC})", S["body_sm"])],
